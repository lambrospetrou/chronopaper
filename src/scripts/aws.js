var CPAws = (function() {
	...
	AWS.config.credentials = new AWS.CognitoIdentityCredentials(buildCognitoCredentialsParams());
	...
	function listTestData() {
		new AWS.S3().getObject({Bucket: 'aws-spito-data', Key: '__auth__/auth.txt'}, function(err, data) {
		  if (err) {
		  	console.error(err);
		  } else {
		  	console.log(data);
		  	document.querySelector('#test-data').innerHTML = '<p>' + data.Body + '</p>'
		  }
		});
	}
